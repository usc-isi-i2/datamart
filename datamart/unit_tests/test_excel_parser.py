...
class TestExcelParser(unittest.TestCase):
    def setUp(self):
        ...
        self.url = "file://" + os.path.abspath(os.path.join(resources_path, "SETA Support DARPA FY2010 and FY2015 Database.xlsx"))
    ...
    def test_parse(self):
        ...
        results = self.parser.parse(self.url)
        ...
        assert_frame_equal(df, sample_result_1)
        ...
        df = pd.read_csv("tmp.csv", index_col=0)
        assert_frame_equal(df, sample_result_2)
        ...
        df = pd.read_csv("tmp.csv", index_col=0)
        assert_frame_equal(df, sample_result_3)
