...
class WorldBankMaterializer(MaterializerBase):
    ...
    def get(self, metadata: dict = None, variables: typing.List[int] = None, constrains: dict = None) -> pd.DataFrame:
        if not constrains:
            constrains = dict()
        date_range = constrains.get("date_range", {"start": "2017","end": "2018"})
...
objwb=WorldBankMaterializer()
dfwb=objwb.get()
dfwb.to_csv('2OUTPUT_CSV_WORLDBANK.csv', sep=',', encoding='utf-8')
