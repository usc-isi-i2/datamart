	    ...
	    for obj in reader:
	    	for element in obj["resources"]:
	    		if element["mimetype"] is None:
		    		...
		    		if("csv" in utf8string and "adc-node1.nal.usda.gov" not in urlline):
		    				# url
		    			...
		    			try:
		   					...
		   					#with open('filename','wb') as f:
		   						#print code
		   					req = urllib2.Request(urllineStr, None, headers)
		   					#print code
		   					code = urllib2.urlopen(req,context = context,timeout=8.0).getcode()#.read()
		    			...
		    			if code == 200:
		   					...
		   					for tag in obj["tags"]:
		   						if tag["name"].encode("utf-8") is None:
		   							pass
		   						else:
		   							data["keywords"].append(tag["name"].encode("utf-8"))
		   					...
		   					for group in obj["groups"]:
		   						...
		   						data["additional_info"]["groups"].append(tmpGroup)
